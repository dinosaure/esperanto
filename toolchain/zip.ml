...
let writestring oc s = output_string oc s
...
let read_ecd filename ic =
  ...
  let rec find_ecd pos len =
    ...
    if pos <= 0L || Int64.sub filelen pos >= 0x10000L then
      raise
        (Error
           (filename, "", "end of central directory not found, not a ZIP file"));
  ...
  try
    ...
    while LargeFile.pos_in ic < cd_bound do
      ...
      if magic <> Int32.of_int 0x02014b50 then
        raise (Error (filename, name, "wrong file header in central directory"));
      ...
      e :=
        {
          ...
          methd =
            ...
            | 0 -> Stored
            | 8 -> Deflated
            | _ -> raise (Error (filename, name, "unknown compression method")));
          mtime = 0.;
          crc;
...
let open_in filename =
  ...
  try
    ...
    let entries =
      read_cd filename ic cd_entries cd_offset (Int32.add cd_offset cd_size)
    in
...
let close_in ifile = Stdlib.close_in ifile.if_channel
...
let blit_from_bytes src ~src_off dst ~dst_off ~len =
  for i = 0 to len - 1 do
    dst.{dst_off + i} <- Bytes.get src (src_off + i)
  done

let substring ba ~off ~len =
  let res = Bytes.create len in
  for i = 0 to len - 1 do
    Bytes.set res i ba.{off + i}
  done;
  ...
  try
    ...
    | Stored ->
        if e.compressed_size <> e.uncompressed_size then
          raise
            (Error (ifile.if_filename, e.filename, "wrong size for stored entry"));
    ...
    | Deflated ->
        ...
        let itmp = De.bigstring_create 0x1000 in
        let otmp = De.bigstring_create 0x1000 in
        let w = De.make_window ~bits:15 in
        let result =
          De.Higher.uncompress ~w
            ~refill:(fun buf ->
              let read =
                input ifile.if_channel tmp 0 (min !in_avail (Bytes.length tmp))
              in
              blit_from_bytes tmp ~src_off:0 buf ~dst_off:0 ~len:read;
              in_avail := !in_avail - read;
              read)
            ~flush:(fun buf len ->
              let buf' = substring buf ~off:0 ~len in
    ...
    | Stored ->
        ...
        let rec copy ctx n =
          if n > 0 then (
            ...
            let ctx = Digest.feed_bytes ctx buf ~off:0 ~len:r in
            output oc buf 0 r;
            copy ctx (n - r))
          else Digest.get ctx
        in
        copy Digest.empty e.uncompressed_size
    | Deflated -> (
        let in_avail = ref e.compressed_size in
        let tmp = Bytes.create 0x1000 in
        let itmp = De.bigstring_create 0x1000 in
        ...
        let result =
          De.Higher.uncompress ~w
            ...
            ~flush:(fun buf len ->
              let buf' = substring buf ~off:0 ~len in
              ctx := Digest.feed_bigstring !ctx buf ~off:0 ~len;
...
let hash_of_filename filename =
  ...
  let rec go ctx =
    match Stdlib.input ic buf 0 (Bytes.length buf) with
    | 0 -> Digest.get ctx
...
let make_symlink_from_entry ifile e =
  let dst = read_entry ifile e in
  `Symlink dst

let copy_entry_to_file ifile e outfilename =
  let oc = open_out_bin outfilename in
  try
    if Int32.logand e.external_attr 0x120000l <> 0l then (
      close_out oc;
      Sys.remove outfilename;
      make_symlink_from_entry ifile e)
    else
      let hash = copy_entry_to_channel ifile e oc in
      let perm = Int32.(to_int (shift_right e.external_attr 16)) land 0o777 in
      Unix.chmod outfilename perm;
      close_out oc;
      `File hash
  with x ->
    close_out oc;
